// app(chat)/api/chat/route.ts v1.4
...
export async function POST(request: Request) {
  ...
  // Передаём в LLM только одно сообщение (текущий запрос)
  const coreMessages = convertToCoreMessages([latestMessage]);

  // Запускаем streamText
  const result = await streamText({
    ...
    // onFinish вызывается, когда модель завершила выдачу
    onFinish: async ({ responseMessages }) => {
      // -- здесь мы считаем токены, сохраняем чат и т.д. --
      ...
      try {
        /**
         * ВАЖНО: `responseMessages` может содержать роли,
         * которые не совпадают с типами, ожидаемыми в БД.
         * Например, "function" или "tool".
         * Если ваша функция sanitizeResponseMessages это не учитывает,
         * можно отредактировать её или привести к нужному типу.
         */
        const cleanedResponseMessages = sanitizeResponseMessages(
          responseMessages // <--- Передаём, как есть
        );
